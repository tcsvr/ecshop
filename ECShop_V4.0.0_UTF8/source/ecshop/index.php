...
$ua = strtolower($_SERVER['HTTP_USER_AGENT']);//获取浏览器信息
...
if (!$smarty->is_cached('index.dwt', $cache_id))//判断是否开启缓存
{
    assign_template();
    ...
    $k = 0;
    foreach ( get_categories_tree() as $v ){
        //echo '<pre>';
        $smarty->assign('index_cate'.$k,$v);
        $k++;
       //print_r($v);
    }
    //exit;
